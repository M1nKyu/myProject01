...
class PlaywrightPDFGenerator:
    ...
    def _initialize_template_env(self):
        """Jinja2 템플릿 환경 설정"""
        try:
            # Flask 앱 컨텍스트 내에서 템플릿 디렉토리 경로 설정
            if current_app:
    ...
    def _load_svg_files(self) -> Dict[str, str]:
        ...
        try:
            if current_app:
                # root_path는 /app/ecoweb/app이므로, static은 /app/ecoweb/static에 있음
                app_root = os.path.dirname(current_app.root_path)
                img_dir = os.path.join(app_root, 'static', 'pdf-reports', 'img')
            else:
                current_dir = os.path.dirname(os.path.abspath(__file__))
                img_dir = os.path.join(current_dir, '..', '..', 'static', 'pdf-reports', 'img')
    ...
    def _get_css_file_path(self, page_number: int) -> str:
        ...
        try:
            if current_app:
                # root_path는 /app/ecoweb/app이므로, static은 /app/ecoweb/static에 있음
                app_root = os.path.dirname(current_app.root_path)
                css_path = os.path.join(app_root, 'static', 'pdf-reports', 'css', f'report{page_number:02d}.css')
            else:
                current_dir = os.path.dirname(os.path.abspath(__file__))
                css_path = os.path.join(current_dir, '..', '..', 'static', 'pdf-reports', 'css', f'report{page_number:02d}.css')
    ...
    def _get_css_content(self, page_number: int) -> str:
        ...
        try:
            ...
            logger.error(f"CSS 파일 읽기 실패: {str(e)}")
            return ""
    ...
    def _get_common_css_content(self) -> str:
        ...
        try:
            if current_app:
                # root_path는 /app/ecoweb/app이므로, static은 /app/ecoweb/static에 있음
                app_root = os.path.dirname(current_app.root_path)
                common_css_path = os.path.join(app_root, 'static', 'pdf-reports', 'css', 'components', 'common.css')
            else:
                current_dir = os.path.dirname(os.path.abspath(__file__))
                common_css_path = os.path.join(current_dir, '..', '..', 'static', 'pdf-reports', 'css', 'components', 'common.css')
            ...
            logger.error(f"공통 CSS 파일 읽기 실패: {str(e)}")
            return ""
    ...
    def _load_page_template(self, page_number: int, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            template_name = f"pdf_reports/report{page_number:02d}.html"
    ...
    def _load_special_page_template(self, page_type: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            template_name = f"pdf_reports/report-00-{page_type}.html"
            template = self.jinja_env.get_template(template_name)
    ...
    def _get_special_page_css_content(self, page_type: str) -> str:
        ...
        try:
            ...
            if current_app:
                # root_path는 /app/ecoweb/app이므로, static은 /app/ecoweb/static에 있음
                app_root = os.path.dirname(current_app.root_path)
                css_path = os.path.join(app_root, 'static', 'pdf-reports', 'css', f'report-00-{page_type}.css')
            else:
                current_dir = os.path.dirname(os.path.abspath(__file__))
                css_path = os.path.join(current_dir, '..', '..', 'static', 'pdf-reports', 'css', f'report-00-{page_type}.css')
